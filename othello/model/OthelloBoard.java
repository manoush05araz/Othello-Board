 ...
 * 
 * Accepts a Visitor which can perform operations on each board cell.
 * 
...
public class OthelloBoard {
	...
	public void set(int row, int col, char token){
		if (validCoordinate(row, col) && (token == P1 || token == P2 || token == EMPTY)){
	...
	public int getCount(char player) {
		TokenCountVisitor visitor = new TokenCountVisitor(player);
		this.accept(visitor);
		return visitor.getCount();
	}
	/** Allow {@link OthelloBoardVisitor} to traverse board.
	* The {@code visitCell} method will be called for every
	* (row, col).
	*/

	public void accept(OthelloBoardVisitor visitor){
		for (int row = 0; row < this.dim; row++){
			for (int col = 0; col < this.dim; col++){
				visitor.visitCell(row, col, this.board[row][col]);
			}
		}
	}
	/**
